...
`_isFullShape` 為 TRUE（全形模式）時，Shift 英數輸出的 ASCII 字元轉換為全形 Unicode 等價字元（`a` → `ａ`、`1` → `１`、`!` → `！`）。

**兩個轉換點**：

| 路徑 | 檔案 | 說明 |
|------|------|------|
| Shift 英數輸出 | `KeyHandler.cpp` `_HandleCompositionShiftEnglishInput()` | `outputChar` 計算後，透過 `FullWidthCharTable` 轉換 |
| 萬用字元直通 | `CompositionProcessorEngine.cpp` `GetCandidateList()` | 單一 `*`/`?` 組字提交時，候選項設為 `＊`/`？`。透過 `_pTextService->_GetFullShape()` 檢查，以 `IsWildcardAllChar()` 判斷字元 |

**非 Shift 非字根鍵**：已由 `_HandleCompositionDoubleSingleByte()` 處理（`KeyEventSink.cpp:226`），透過 `FullWidthCharTable` 轉換，無需額外修改。

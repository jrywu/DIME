...
LangString DESC_VCARM64_DECISION ${LANG_TradChinese} "安裝此輸入法之前，必須先安裝 $(DESC_VCARM64)，若你想繼續安裝 \
  ，您的電腦必須連接網路。$\n您要繼續這項安裝嗎？"
!define URL_VC_REDISTX64 https://aka.ms/vs/14/release/vc_redist.x64.exe
!define URL_VC_REDISTX86 https://aka.ms/vs/14/release/vc_redist.x86.exe
!define URL_VC_REDISTARM64 https://aka.ms/vs/14/release/vc_redist.arm64.exe
...
Function .onInit
  ...
  MessageBox MB_OKCANCEL|MB_ICONQUESTION "偵測到舊版 $0，建議您先移除舊版再安裝新版。是否要現在移除？" IDOK +2
  	Abort
  ...
  IfFileExists "$SYSDIR\DIME.dll"  0 RemoveFinished      ;代表反安裝失敗
        Abort
  RemoveFinished:
    	MessageBox MB_ICONINFORMATION|MB_OK "舊版已移除完成。"
StartInstall:
...
Section -Post
  ...
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayVersion" "${PRODUCT_VERSION}.${PRODUCT_SUBVERSION}"
...
Function un.onUninstSuccess
  MessageBox MB_ICONINFORMATION|MB_OK "$(^Name)已經從您的電腦中移除完成。" /SD IDOK
FunctionEnd

Function un.onInit
;!insertmacro MUI_UNGETLANGUAGE
  MessageBox MB_ICONQUESTION|MB_YESNO|MB_DEFBUTTON2 "您確定要完全移除 $(^Name)嗎？" /SD IDYES IDYES +2
  Abort
FunctionEnd

...
1. 下載安裝檔
   ...
   **最新穩定發行版本 DIME v.1.3.508（更新日期: 2026-03-16）**
   ...
   <!-- DOWNLOAD_START -->     
     <details>
     <summary><b>最新開發版本 DIME v1.3.512 (更新日期: 2026-03-16)</b></summary>
   <!-- DOWNLOAD_END -->
...
3. **（建議）驗證檔案完整性：**
   ...
   **最新穩定發行版本 DIME v.1.3.508 SHA-256 CHECKSUM (更新日期: 2026-03-16):**
   <!-- RELEASE_CHECKSUM_START -->    
    | 檔案 | SHA-256 CHECKSUM |
    |------|----------------|
    | DIME-Universal.exe | `7440B002E56A683E868A64B2883436A16728490940BDDD3AD0F83CA5609F9E07` |
    | DIME-Universal.zip | `A0EB2B3BA9BDD4FEDE5CEC955EBB598BC3B83129711226E97F645BB53A54FB1F` |
    | DIME-64bit.msi | `749CAFC42A99924DC0261F56CC7A952090F1AC3B470E1435545EFAC19D602C7D` |
    | DIME-32bit.msi | `300F7C9AC4167479B5545A4BB1E0C6FB7EEB42031AD57FC16A543BA92F62B814` |
   <!-- RELEASE_CHECKSUM_END -->   

   <!-- CHECKSUM_START -->
     <details>     
     <summary><b>最新開發版本 DIME v1.3.512 SHA-256 CHECKSUM (更新日期: 2026-03-16)</b></summary>
   
    | 檔案 | SHA-256 CHECKSUM |
    |------|----------------|
    | DIME-Universal.exe | `D500BB93F299903FDBFF2834F480EF2394A9D7C31D2D2E6A6CBDF3D6AA292951` |
    | DIME-Universal.zip | `EC1A4335CEC7C87CC8963F1887F878CEDFDA30BD4ED5D62D59C3B054B18CAFC1` |
    | DIME-64bit.msi | `1C3D51DAB7A7D0A4337DF294FE3C5FBD0568262781E12A403EC1D6F10FDA3660` |
    | DIME-32bit.msi | `3A4BE9AB7EFE924C83851406582553EBCBB50B4C3B4EDBE6FD35A62431153E9D` |
   <!-- CHECKSUM_END -->
     </details>
...
在中文輸入模式下，未開始組字時，按住 Shift 鍵可直接輸入英數字元，無需切換至英文模式。所有可列印 ASCII 字元（字母、數字、符號）在按住 Shift 時會完全繞過輸入法引擎，直接輸出對應字元。組字期間（輸入緩衝區非空時），Shift 英數輸入模式不作用，所有按鍵皆由輸入法引擎處理。

> **萬用字元與 Shift 英數的互動**：在大易、行列等輸入法中，Shift+8（`*`）與 Shift+/（`?`）同時也是萬用字元按鍵。若作為 Shift 英數連續輸入的第一個按鍵，會進入輸入法引擎啟動萬用字元搜尋；若已在連續輸入過程中，則依 CapsLock 狀態正常輸出字元（CapsLock OFF 輸出 `*` 和 `?`；CapsLock ON 輸出 `8` 和 `/`），不會觸發萬用字元功能。注音輸入法不支援 `*` 萬用字元，因此 Shift+8 在注音模式下一律作為 Shift 英數輸出。

**CapsLock 控制輸出字元：**

所有按鍵依 CapsLock 狀態決定輸出：

| 按鍵類型 | CapsLock OFF | CapsLock ON | 範例 |
|---------|--------------|-------------|------|
| 字母鍵 | 小寫字母 | 大寫字母 | Shift+A → `a` / `A` |
| 數字鍵（`1`~`0`） | Shift 字元 | 基礎字元 | Shift+1 → `!` / `1`、Shift+2 → `@` / `2` |
| 其他符號鍵 | 基礎字元 | Shift 字元 | Shift+- → `-` / `_` |

> **數字鍵與字母鍵的差異**：數字鍵（`1`~`0`）的 CapsLock 行為與字母鍵相反。CapsLock OFF 時按住 Shift 輸出 Shift 字元（`!@#$%^&*()`）；CapsLock ON 時則回復為基礎數字（`1234567890`）。若需在中文輸入模式下輸入阿拉伯數字，建議將右方九宮格數字鍵盤設定為「數字輸入」模式（在設定頁面將「九宮格數字鍵盤」調整為數字輸入），直接按數字小鍵盤即可輸入數字。

**使用範例：**

- **中英混合輸入**：輸入「請使用」→ 按住 Shift 輸入 ` git commit ` → 輸入「命令」，即可得到「請使用 git commit 命令」。按住 Shift 期間可連續輸入英數字元及空格（Shift+Space 直接產生空格）。
- **電子郵件地址**：確認 CapsLock OFF，按住 Shift 連續輸入 `user`、`2`（產生 `@`）、`example.com`，即可得到 `user@example.com`（CapsLock OFF 時，Shift+字母 = 小寫字母、Shift+2 = `@`）。
- **混合英數符號與數字輸入**：輸入「版本」→ 按住 Shift 輸入 `v` → 放開 Shift（結束 Shift 英數模式）→ 在九宮格數字鍵盤（設定為數字輸入模式）按 `1`、`.`、`2` → 按住 Shift 輸入 `-beta` → 輸入「已發布」，即可得到「版本v1.2-beta已發布」。九宮格數字鍵盤輸入的數字不受 Shift 英數模式影響，在設定輸入輸入模式時可隨時用來輸入數字。
...
| 行列30 Unicode Ext-A~G | 查詢行列30 CJK 基本區與擴充 A~G 區 |
